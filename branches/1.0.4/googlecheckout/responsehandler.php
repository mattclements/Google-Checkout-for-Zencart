...
function process_order_state_change_notification($root, $data, $message_log) {
    ...
    switch($new_fulfillment_order) {
      ...
      case 'DELIVERED': {  
    global $db;	  
	$goid = $data[$root]['google-order-number'];
    $db_query = $db->Execute("select orders_id from google_orders where google_order_number = '$goid'");
    $order_id = $db_query->fields['orders_id'];
    ...
    $sql_data_array = array('orders_status' => 3,
	                           'last_modified' => 'now()');	    
  ...
  function process_charge_amount_notification($root, $data, $message_log) {
    ...
    $sql_data_array = array('orders_id' => $order_id,
                           ...
                           'date_added' => 'now()',
                           'customer_notified' => 1,
                           'comments' => 'Charge initiated to the customer credit card.');
    zen_db_perform(TABLE_ORDERS_STATUS_HISTORY, $sql_data_array);	    
    $sql_data_array = array('orders_status' => 2,
	                           'last_modified' => 'now()');
    zen_db_perform(TABLE_ORDERS, $sql_data_array, 'update', "orders_id = '" . $order_id . "'");  
    send_ack();
  }
