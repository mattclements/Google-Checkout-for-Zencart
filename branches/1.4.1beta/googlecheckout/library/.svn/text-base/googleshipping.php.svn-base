...
//    var $address_filters;
     ...
     * @param string $shipping_type the shipping option, valid values are 
     *                              "ONE_DAY", "TWO_DAY", "THREE_DAY", "GROUND"
    ...
    function GoogleCarrierCalculatedShippingOption($price, $shipping_company,
      ...
      $this->shipping_type = strtoupper(trim($shipping_type));
...
//    }
  ...
  class GoogleShippingPackage {
     ...
     * @param string $packaging the type of packaging that will be used, valid
     * values are "BOX", "CARRIERBOX", "CARRIERPAK", "CARRIERTUBE", 
     * "CARRIERENVELOPE" or "MEDIAMAIL". Defaults to "BOX".
     */
    function GoogleShippingPackage($ship_from, $width, $length, $height, $unit,
                    $delivery_address_category='RESIDENTIAL', $packaging='BOX') {
      ...
      switch(strtoupper($unit)){
        case 'RESIDENTIAL':
        case 'COMMERCIAL':
          $this->delivery_address_category = $delivery_address_category;
          break;
        default:
          $this->delivery_address_category = 'RESIDENTIAL';
      }
      
      switch(strtoupper($packaging)){
        case 'BOX':
        case 'CARRIERBOX':
        case 'CARRIERPACK':
        case 'CARRIERTUBE':
        case 'CARRIERENVELOPE':
        case 'MEDIAMAIL':
          $this->packaging = strtoupper($packaging);
          break;
        default:
          $this->packaging = 'Box';
      }
