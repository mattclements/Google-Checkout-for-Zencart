...
//          $xml_data->Push($ship->type, array('name' => $ship->name));
          ...
          $ShippingPackages = $ship->ShippingPackages;
          $xml_data->Push('shipping-packages');
          foreach($ShippingPackages as $ShippingPackage){
            $xml_data->Push('shipping-package');
            $xml_data->Push('ship-from', array('id' => $ShippingPackage->ship_from->id));
            $xml_data->Element('city', $ShippingPackage->ship_from->city);
            $xml_data->Element('region', $ShippingPackage->ship_from->region);
            $xml_data->Element('postal-code', $ShippingPackage->ship_from->postal_code);
            $xml_data->Element('country-code', $ShippingPackage->ship_from->country_code);
            $xml_data->Pop('ship-from');

            $xml_data->EmptyElement('width', array('unit' => $ShippingPackage->unit,
                                           'value' => $ShippingPackage->width
                                            ));
            $xml_data->EmptyElement('length', array('unit' => $ShippingPackage->unit,
                                            'value' => $ShippingPackage->length
                                            ));
            $xml_data->EmptyElement('height', array('unit' => $ShippingPackage->unit,
                                            'value' => $ShippingPackage->height
                                            ));
            
            $xml_data->Element('delivery-address-category',
                  $ShippingPackage->delivery_address_category);

            $xml_data->Element('packaging',
                  $ShippingPackage->packaging);
