      ...
      if($customer_info->RecordCount() == 0)  {
        ...
        $sql_data_array = array('customers_gender' => '',
                        'customers_firstname' => $data[$root]['buyer-shipping-address']['contact-name'],
                          'customers_lastname' => '',
                          'customers_dob' => $data[$root]['timestamp'],
        ...
        $sql_data_array = array('customers_info_id' => $cust_id,
                            ...
                            'customers_info_date_account_created' => $data[$root]['timestamp'],
                            'customers_info_date_account_last_modified' => $data[$root]['timestamp'],
...
//BOF - Fix address book bug - added by colosports
    ...
    $zone_id = $zone_answer->fields['zone_id'];
    $zone_country_id = $zone_answer->fields['zone_country_id'];

/* Removed because makeSqlInteger($zone_answer['zone_country_id']) returns NULL;
...
// Define link to Google order and order number.
	if (MODULE_PAYMENT_GOOGLECHECKOUT_MODE=='https://checkout.google.com/'){
		$google_link = '<a href="https://checkout.google.com/order?t=' . $data[$root]['google-order-number'] . '" target="_blank">' . $data[$root]['google-order-number'] . '</a>';
		}
	else {
		$google_link = '<a href="https://sandbox.google.com/order?t=' . $data[$root]['google-order-number'] . '" target="_blank">' . $data[$root]['google-order-number'] . '</a>';
		}
    ...
    $sql_data_array = array('customers_id' => $cust_id,
                           ...
                           'payment_method' => 'Google No: ' . $google_link,
