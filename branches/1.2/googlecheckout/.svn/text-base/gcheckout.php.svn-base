...
// $products_description = $products_description->fields['products_description']; 
    ...
    if(!empty($tt))
		...
		$gcheck->element('merchant-item-id', base64_encode(serialize($products[$i]['id'])));
...
//		$flat_array = tep_db_fetch_array($check_query);
	...
	if ($zone != '') {
		$zone_result = $db->Execute("select countries_name, zone_code 
		...
		while(!$zone_result->EOF){
			...
			$allowed_restriction_country[] = $zone_result->fields['countries_name'];
