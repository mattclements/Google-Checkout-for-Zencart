...
//      $sql = $db->bindVars($sql, ':couponIDEntered', , 'string');
	...
	// invalid discount coupon code or more than one entered!
	      ...
	      $coupons = new GoogleCoupons("false", $curr_code['code'],0, $_SESSION['currency'], $text_coupon_help);
...
// BBG End
	...
	// valid discount coupon code
        ...
        $coupons = new GoogleCoupons("true", $curr_code['code'],$currencies->get_value($_SESSION['currency']) * $coupon_amount,$_SESSION['currency'], $text_coupon_help);
        $merchant_result->AddCoupons($coupons);
        $first_coupon = false;
	    }
...
//            Compute the price for this shipping method and address id
	            ...
	            $merchant_result->SetShippingDetails($name, $currencies->get_value($_SESSION['currency']) * $price, $_SESSION['currency'], $shippable);
	
	            if($data[$root]['calculate']['tax']['VALUE'] == "true") {
	              //Compute tax for this address id and shipping type
	              $amount = 15; // Modify this to the actual tax value
	              $merchant_result->SetTaxDetails($currencies->get_value($_SESSION['currency']) * $amount, $_SESSION['currency']);
	            }
...
//	  	$product_id = unserialize(base64_decode($item['merchant-item-id']['VALUE']));
    ...
    for($i=0; $i<sizeof($products); $i++) {
      ...
      $sql_data_array = array('orders_id' => $orders_id,
                          ...
                          'products_price' => makeSqlFloat($currencies->get_value($_SESSION['currency']) * $products[$i]['price']),
                          'final_price' => makeSqlFloat($currencies->get_value($_SESSION['currency']) * $products[$i]['final_price']),
      ...
      if (isset($products[$i]['attributes']) && is_array($products[$i]['attributes']))  {
        while (list($option, $value) = each($products[$i]['attributes'])) {
          ...
          $sql_data_array = array('orders_id' => $orders_id,
                          ...
                          'options_values_price' => makeSqlFloat($currencies->get_value($_SESSION['currency']) * $attributes->fields['options_values_price']),
